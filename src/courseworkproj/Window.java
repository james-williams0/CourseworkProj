...
public class Window extends JFrame {
    ...
    //  Original initialisations of the stack and queue.
    private DrawStack drawableStack = new DrawStack();
    private DrawQueue drawableQueue = new DrawQueue();

    //  Boolean flags for whether the application is currently drawing the stack or queue
    ...
    private class Canvas extends JPanel {
        ...
        public void paintComponent(Graphics g) {
            ...
            if(onStack) {
                drawableStack.draw(g, canvas);
                onStack = false;
            } else if(onQueue) {
                drawableQueue.draw(g, canvas);
                onQueue = false;
            } else {
                FileLoad("stack.txt", null, false);
                drawableStack.draw(g, canvas);
            }
        }
    }

    //  Action listener for all buttons except the top menu and the toggle button for selecting between the queue and the stack.
    //  This listener essentially uses lots of selection flow control structures to get the desired result depending on the source of the action.
